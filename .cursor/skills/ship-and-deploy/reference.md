...
| **K8s GitOps (Argo CD)** | push image-tag changes into `k8s/overlays/*/kustomization.yaml`; Argo reconciles | Argo app health + ingress health checks | Keep secrets out of git; migration hook runs PreSync in prod |
...
| Workflow | Purpose |
| --- | --- |
| `.github/workflows/cd-build-dev.yml` | Build backend + dev/prod frontend artifacts, push to GHCR, auto-update dev overlay tags |
